...
console.log('screens:', (await ev(`JSON.stringify({shelf: document.getElementById('screen-shelf').hidden, reader: document.getElementById('screen-reader').hidden})`)).v);
console.log('restore调用:', (await ev(`JSON.stringify(JSON.parse(NovelBridge.tryRestoreExternal()))`)).v);
console.log('lastBook:', JSON.stringify((await ev(`NovelBridge.loadLastBook()`)).v));
console.log('progress:', JSON.stringify((await ev(`NovelBridge.loadProgress('a:哇！爆率真的很高.txt')`)).v || '').slice(0, 150));
